...
const actionLabels: Record<string, string> = {
  ...
  mentioned: '@提到了你',
  announcement: '系统公告',
  workflow_task_assigned: '分配了工作流任务',
  workflow_completed: '工作流已通过',
          ...
          renderItem={(item) => (
            <List.Item
              ...
              onClick={async () => {
                ...
                // 工作流通知点击跳转到流程详情
                if (item.action?.startsWith('workflow_') && item.metadata) {
                  try {
                    const meta = typeof item.metadata === 'string' ? JSON.parse(item.metadata) : item.metadata
                    if (meta.link) {
                      router.visit(meta.link)
                    }
                  } catch {
                    // 静默失败
                  }
            ...
            >
              <Space orientation="vertical" size={2} style={{ width: '100%' }}>
                {item.action === 'workflow_task_assigned' && item.metadata ? (
                  <>
                    <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: 13 }}>
                      <Text strong style={{ color: '#1677FF' }}>您有一个新的待办任务</Text>
                    </Text>
                    <Text type="secondary" style={{ fontSize: 12 }}>
                      {(() => {
                        try {
                          const meta = typeof item.metadata === 'string' ? JSON.parse(item.metadata) : item.metadata
                          return `${meta.workflow_name || ''} - ${meta.task_name || ''}`
                        } catch { return '' }
                      })()}
                    </Text>
                  </>
                ) : item.action === 'workflow_completed' && item.metadata ? (
                  <>
                    <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: 13 }}>
                      <Text strong style={{ color: '#52c41a' }}>工作流已通过</Text>
                    </Text>
                    <Text type="secondary" style={{ fontSize: 12 }}>
                      {(() => {
                        try {
                ...
                ) : item.action === 'workflow_cancelled' && item.metadata ? (
                  <>
                    <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: 13 }}>
                      <Text strong style={{ color: '#faad14' }}>工作流已取消</Text>
                    </Text>
                    <Text type="secondary" style={{ fontSize: 12 }}>
                      {(() => {
                        try {
