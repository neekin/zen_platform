...
export default function Dashboard({ stats, chart_data, recent_activities, framework }: DashboardProps) {
  ...
  const eventLabels: Record<string, string> = {
    create: t('dashboard.eventCreate'),
    update: t('dashboard.eventUpdate'),
    destroy: t('dashboard.eventDestroy'),
  }
  ...
  return (
    ...
    >
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        <Col span={24}>
          <RealtimeTrendChart
            ...
            title={t('dashboard.realtimeTrend')}
            yFieldLabel={t('dashboard.requestCount')}
            color="#D4A537"
          />
        </Col>
      </Row>
      ...
      {recent_activities.length > 0 && (
        <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
          <Col span={24}>
            <ProCard title={t('dashboard.recentActivity')} variant="borderless" className="glass-card">
              <Table
                size="small"
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        <Col xs={24} lg={12}>
          <ProCard title={t('dashboard.quickStart')} variant="borderless" className="glass-card">
            <Space orientation="vertical" size={12} style={{ width: '100%' }}>
              <div>
                <Tag color="blue">1</Tag>
                <Text>{t('dashboard.defineModelDsl')}</Text>
              </div>
              <div>
                <Tag color="blue">2</Tag>
                <Text>{t('dashboard.generateCrud')} — </Text>
              ...
              <div>
                ...
                <Text>{t('dashboard.generateApi')} — </Text>
        ...
        <Col xs={24} lg={12}>
          <ProCard title={t('dashboard.docsTitle')} variant="borderless" className="glass-card">
            <Space orientation="vertical" size={12} style={{ width: '100%' }}>
              <Link href="https://zen.justfunit.net" target="_blank">
                <BookOutlined /> {t('dashboard.userDocs')}
              </Link>
              <Link href="/api-docs">{t('dashboard.apiDocs')}</Link>
              <Link href="https://github.com/neekin/zen_platform" target="_blank">{t('dashboard.githubRepo')}</Link>
            </Space>
          </ProCard>
        </Col>
      </Row>
      ...
      {/* 自定义统计指南 */}
      <ProCard title={t('dashboard.howToAddStats')} variant="borderless" className="glass-card" style={{ marginTop: 16 }}>
        <Alert
          type="info"
          showIcon
          title={t('dashboard.customDashboardStats')}
          description={t('dashboard.editDashboardDesc')}
